// Persistent Segment Tree
#define M ((l + r) >> 1)
struct Node{
    Node *left, *right;
    ll val;
    Node(ll x) : left(NULL), right(NULL), val(x) {}        // Add value
    Node(Node *l, Node *r) : left(l), right(r), val(0) {    // Update values
        if(l) val += l->val;
        if(r) val += r->val;
    }
    Node(Node *root) : left(root->left), right(root->right), val(root->val) {}    // Make copy
};
Node *build(int l, int r, vector<int> &values) {
    if(l == r) return new Node(values[l]);
    return new Node(build(l, M, values), build(M+1, r, values));
}
Node *update(Node *node, int l, int r, int idx, ll k) {
    if(l == r) return new Node(k);
    if(idx <= M) return new Node(update(node->left, l, M, idx, k), node->right);
    return new Node(node->left, update(node->right, M+1, r, idx, k));
}
ll query(Node *node, int l, int r, int a, int b) {
    if(a > r || b < l) return 0;
    if(a <= l && r <= b) return node->val;
    return query(node->left, l, M, a, b) + query(node->right, M+1, r, a, b);
}
Node *roots[N];
// 0 based indexing
// roots[copy++] = build(0, n-1);
// roots[copy++] = new Node(roots[--idx]);   
// roots[--copy] = update(roots[--copy], 0, n-1, --idx, x);
// query(roots[--copy], 0, n-1, --a, --b)